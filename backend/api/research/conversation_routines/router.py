...
async def get_opening_suggestions(
    user: User = Depends(get_current_user),
    prefer_audiences: Optional[str] = None,
    prefer_regions: Optional[str] = None,
    industries: Optional[str] = None,
    min_words: int = 14,
    max_words: int = 22,
):
    """Return rotating first-turn suggestions generated by LLM in natural sentence form.
    Query params:
      - prefer_audiences: comma-separated (e.g., "venture,B2B SaaS,fintech")
      - prefer_regions: comma-separated (e.g., "DACH,UK,EU")
      - industries: comma-separated (e.g., "fintech,logistics,healthcare")
      - min_words/max_words: integer bounds
    """
    try:

        def split_param(s: Optional[str]):
            return [p.strip() for p in s.split(",") if p.strip()] if s else None

        audiences = split_param(prefer_audiences)
        # Default rotation when no prefer_regions provided
        regions = split_param(prefer_regions) or [
            random.choice(["EU", "DACH", "US", "UK"])
        ]
        inds = split_param(industries)

        suggestions = await conversation_service.generate_opening_suggestions(
            audiences=audiences,
            regions=regions,
            industries=inds,
            min_words=min_words,
        ...
        logger.error(f"🔴 Failed to generate opening suggestions: {e}")
        return {
            "suggestions": [
