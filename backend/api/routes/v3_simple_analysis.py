...
async def analyze_context_enhanced(
    ...
    try:
        ...
        context_result = await _extract_context_with_llm(
            llm_service, conversation_context, latest_input, existing_context
        )
        ...
        logger.debug(
            f"Context analysis completed with confidence: {confidence_score:.2f}"
        )
...
async def analyze_intent_enhanced(
    ...
    try:
        ...
        llm_service = LLMServiceFactory.create("enhanced_gemini")
        intent_result = await _analyze_intent_with_llm(
        ...
        logger.debug(
            f"Intent analysis completed: {intent_result.get('intent', 'unknown')} (confidence: {confidence_score:.2f})"
        )
...
async def validate_business_readiness(
    service, conversation_context: str, latest_input: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not context_analysis:
            ...
            business_idea = (
                latest_input if len(latest_input) > 10 else "business idea mentioned"
            )
        ...
        has_business_idea = bool(
            business_idea and business_idea.strip() and len(business_idea.strip()) > 10
        )
        ...
        user_wants_questions = any(
            keyword in latest_input.lower() for keyword in intent_keywords
        )
        ...
        readiness_score = max(
            0.5, sum(score_components)
        )  # Minimum 0.5 if user wants questions
        ...
        validation_result = {
            ...
            "strengths": (
                ...
                else ["user_intent"] if user_wants_questions else []
            ),
            "recommendations": (
                ["Define target customer", "Identify key problems"]
                if ready_for_questions
                else ["Define your business idea clearly"]
            ),
            "reasoning": f"V1 Sustainable validation: Business idea: {has_business_idea}, User wants questions: {user_wants_questions}",
        }

        logger.info(
            f"✅ V1 Sustainable validation: ready={ready_for_questions} (score: {readiness_score:.2f})"
        )
        logger.info(
            f"  - Business idea: {has_business_idea} ({len(business_idea) if business_idea else 0} chars)"
        )
...
def _calculate_context_confidence(context_result: Dict[str, Any]) -> float:
    ...
    try:
        business_idea = context_result.get("businessIdea") or context_result.get(
            "business_idea", ""
        )
        target_customer = context_result.get("targetCustomer") or context_result.get(
            "target_customer", ""
        )
        ...
        customer_score = (
            min(1.0, len(str(target_customer)) / 30) if target_customer else 0.0
        )
...
def _analyze_business_clarity(context_result: Dict[str, Any]) -> Dict[str, float]:
    ...
    try:
        ...
        return {
            "idea_clarity": (
                min(1.0, len(str(business_idea)) / 50) if business_idea else 0.0
            ),
            "customer_clarity": (
                min(1.0, len(str(target_customer)) / 30) if target_customer else 0.0
            ),
...
def _analyze_conversation_stage(
    messages: List[Dict[str, Any]], intent_result: Dict[str, Any]
) -> str:
...
def _calculate_intent_confidence(
    intent_result: Dict[str, Any], messages: List[Dict[str, Any]]
) -> float:
    ...
    try:
        ...
        intent_clarity = (
            0.8 if intent in ["question_request", "continue_conversation"] else 0.5
        )
...
async def analyze_industry_enhanced(
    service, conversation_context: str, context_analysis: Dict[str, Any]
    ...
    try:
        ...
        llm_service = LLMServiceFactory.create("enhanced_gemini")
        industry_result = await _classify_industry_with_llm(
            llm_service, conversation_context, context_analysis
        )

        # V3 Enhancement: Add sub-category analysis
        if industry_result:
            sub_categories = _analyze_industry_subcategories(
                industry_result, context_analysis
            )
        ...
        logger.debug(
            f"Industry analysis completed: {industry_result.get('industry', 'unknown') if industry_result else 'none'}"
        )
...
async def detect_stakeholders_enhanced(
    ...
    try:
        ...
        context_hash = hashlib.md5(
            f"stakeholders:{business_idea}:{target_customer}".encode()
        ).hexdigest()
        ...
        if stakeholder_result:
            ...
            # Store confidence in metrics
            service.metrics.confidence_scores["stakeholder_detection"] = (
                confidence_score
            )
...
async def analyze_conversation_flow(
    ...
    try:
        ...
        flow_result = {
            "current_stage": _determine_conversation_stage(
                messages, context_analysis, intent_analysis
            ),
            "next_recommended_action": _recommend_next_action(
                messages, context_analysis, intent_analysis
            ),
            "conversation_quality": _assess_conversation_quality(
                messages, context_analysis
            ),
            "readiness_for_questions": _assess_question_readiness(
                messages, context_analysis, intent_analysis
            ),
        ...
        logger.debug(
            f"Conversation flow analysis completed: stage={flow_result['current_stage']}, action={flow_result['next_recommended_action']}"
        )
        return flow_result
...
def _analyze_industry_subcategories(
    industry_result: Dict[str, Any], context_analysis: Dict[str, Any]
) -> List[str]:
    ...
    try:
        ...
        if business_idea:
            ...
            relevant_subcategories = [
                ...
                if any(word in business_lower for word in sub.split())
            ]
...
def _prioritize_stakeholders(
    stakeholder_result: Dict[str, Any], context_analysis: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        for stakeholder in stakeholders:
            ...
            if (
                target_customer.lower() in stakeholder_name.lower()
                or stakeholder_name.lower() in target_customer.lower()
            ):
...
def _calculate_stakeholder_confidence(stakeholder_result: Dict[str, Any]) -> float:
    ...
    try:
        ...
        stakeholder_count_score = min(
            1.0, len(stakeholders) / 5
        )  # Optimal around 5 stakeholders
...
def _assess_conversation_quality(
    messages: List[Dict[str, Any]], context_analysis: Dict[str, Any]
) -> float:
    ...
    try:
        ...
        message_count_score = min(
            1.0, len(user_messages) / 5
        )  # Optimal around 5 messages
...
def _assess_question_readiness(
    messages: List[Dict[str, Any]],
    context_analysis: Dict[str, Any],
    intent_analysis: Dict[str, Any],
) -> bool:
...
def _assess_engagement_level(messages: List[Dict[str, Any]]) -> float:
    ...
    try:
        ...
        frequency_score = min(
            1.0, len(user_messages) / 10
        )  # Optimal around 10 messages
...
async def _validate_business_readiness_with_llm(
...
async def _classify_industry_with_llm(
    llm_service, conversation_context: str, context_analysis: Dict[str, Any]
) -> Dict[str, Any]:
...
async def _detect_stakeholders_with_llm(
    llm_service,
    conversation_context: str,
    context_analysis: Dict[str, Any],
    ...
    try:
        ...
        return {
            "primary": [
                ...
                {
                    "name": "Customers",
                    "description": "People who purchase the product/service",
                },
            ],
            "secondary": [
                {
                    "name": "Decision Makers",
                    "description": "People who influence purchasing decisions",
                }
            ],
            "confidence": 0.3,
