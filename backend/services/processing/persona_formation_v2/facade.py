...
from backend.services.processing.persona_formation_v2.validation import PersonaValidation
...
class PersonaFormationFacade:
    ...
    def _make_persona_from_attributes(self, attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        extracted = {
            ...
            "challenges_and_frustrations": self.challenges_ex.from_attributes(attributes),
    ...
    async def generate_persona_from_text(
        ...
        if isinstance(text, list) and text and isinstance(text[0], dict) and "dialogue" in text[0]:
            return await self.form_personas_from_transcript(text, context=context)

        # Otherwise, structure the transcript first
        filename = (context or {}).get("filename") if isinstance(context, dict) else None
        segments = await self.structuring.structure_transcript(str(text), filename=filename)
        if not segments:
            return []
    ...
    async def form_personas_from_transcript(
        ...
        for seg in transcript:
            try:
                ...
                by_speaker.setdefault(speaker, []).append(seg.get("dialogue") or seg.get("text") or "")
            except Exception:
                continue
        ...
        for speaker, utterances in by_speaker.items():
            ...
            # Extract attributes (AttributeExtractor internally applies EVIDENCE_LINKING_V2 when enabled)
            scope_meta = {"speaker_id": speaker, "document_id": (context or {}).get("document_id")}
            attributes = await self.extractor.extract_attributes_from_text(
                scoped_text, role="Participant", scope_meta=scope_meta
            )
            persona = self._make_persona_from_attributes(attributes)
        ...
        if not personas:
            ...
            attributes = await self.extractor.extract_attributes_from_text(all_text, role="Participant")
            personas = [self._make_persona_from_attributes(attributes)]
