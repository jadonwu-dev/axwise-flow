...
class AdminService:
    ...
    ADMIN_USER_IDS = os.getenv("ADMIN_USER_IDS", "user_2xdZVfEfNCMGdl6ZsBLYYdCoWIw").split(",")
    ...
    def is_admin(self, user: User) -> bool:
        ...
        try:
            CLERK_...=***REMOVED***
            if clerk_user and clerk_user.get("publicMetadata", {}).get("role") == "admin":
                return True
    ...
    def create_admin_trial(self, user: User, duration_days: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        if existing_trials >= self.MAX_ADMIN_TRIALS_PER_USER:
            raise ValueError(f"Maximum admin trials ({self.MAX_ADMIN_TRIALS_PER_USER}) exceeded")
        ...
        # Log admin action
        logger.info(f"Admin trial created: user={user.user_id}, trial_id={trial_id}, duration={duration}d")
    ...
    def revoke_admin_trial(self, user: User) -> bool:
        ...
        if user.usage_data and "subscription" in user.usage_data:
            ...
            user.usage_data["subscription"]["canceled_at"] = datetime.now(timezone.utc).isoformat()
