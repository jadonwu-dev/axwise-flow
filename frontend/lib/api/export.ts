...
export async function exportAnalysisPdf(analysisId: string): Promise<Blob> {
  try {
    console.log(`Exporting analysis ID: ${analysisId} as PDF`);
    
    // Make the API call with responseType blob to get binary data
    const response = await apiCore.getClient().get(`/api/export/${analysisId}/pdf`, {
      responseType: 'blob',
 ...
 */
export function getPdfExportUrl(analysisId: string): string {
  const baseUrl = apiCore.getBaseUrl();
  return `${baseUrl}/api/export/${analysisId}/pdf`;
}
